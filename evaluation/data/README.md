...
```python
from evaluation.fund_search_eval_dataset import FUND_SEARCH_EXAMPLES
...
```python
from evaluation.fund_search_eval_dataset import load_future_feature_examples
...
### View Statistics
```bash
uv run python -m evaluation.fund_search_eval_dataset
```
...
Run evaluation:
```bash
MLFLOW_ENABLED=true uv run python -m evaluation.evaluate
```
...
```bash
cd evaluation/data/_current_schema/dual_criteria
...
```bash
cd evaluation/data/_future_features/single_sector
# Edit queries.jsonl, add new line:
